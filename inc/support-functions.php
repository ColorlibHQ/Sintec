...
// social media
if ( ! function_exists( 'sintec_social' ) ) {
	function sintec_social( $args = array()  ){
		...
		$url = sintec_opt('sintec_social_url');
...
function sintec_contact_form7_shortcode(){

    // contact form list
    $getforms['cs'] = __( 'Custom Shortcode', 'sintec' );
    // Instruction
    $Instruction = ''; 
    ...
    if( defined('WPCF7_VERSION') ){
        ...
        if( $loop->have_posts() ){
            ...
            $Instruction = __( 'Contact form not found.', 'sintec' );
        }
    }else{
        $url = admin_url( 'plugins.php' );
        
        $Instruction = sprintf( __( 'If you want to use contact form 7, Please install and active contact form 7 plugin. %s Click here to install %s  ' , 'sintec' ), '<a target="_blank" href="'.esc_url( $url ).'">', '</a>'  );
    }
...
// Popular post count
function sintec_set_post_views($postID) {
    $count_key = 'sintec_post_views_count';
...
// blog post categoty 
function sintec_get_post_cat(){
    $cats = get_categories();

    $categories = array( 'na' => esc_html__( 'Select post category', 'sintec' ) );
