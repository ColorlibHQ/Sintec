...
/*=========================================================
	Image Crop Size
=========================================================*/
add_image_size( 'sintec_blog_750x375', 750, 375, true );
add_image_size( 'sintec_widget_post_thumb', 80, 80, true );
add_image_size( 'sintec_np_thumb', 60, 60, true );
add_image_size( 'sintec_latest_blog_360x285', 360, 285, true );
add_image_size( 'project_image', 560, 420, true );
add_image_size( 'review_img', 100, 100, true );


/*=========================================================
	Theme option callback
=========================================================*/
function sintec_opt( $id = null, $default = '' ) {
...
=========================================================*/
function sintec_meta( $id = '' ){
    
    $value = get_post_meta( get_the_ID(), '_sintec_'.$id, true );
...
/*=========================================================
	Blog Date Permalink
=========================================================*/
function sintec_blog_date_permalink(){
...
/*========================================================
	Blog Excerpt Length
========================================================*/
if ( ! function_exists( 'sintec_excerpt_length' ) ) {
	function sintec_excerpt_length( $limit = 30 ) {
...
==========================================================*/
if ( ! function_exists( 'sintec_posted_comments' ) ) {
    function sintec_posted_comments(){
        
        $comments_num = get_comments_number();
        if( comments_open() ){
            if( $comments_num == 0 ){
                $comments = esc_html__('No Comments','sintec');
            } elseif ( $comments_num > 1 ){
                $comments= $comments_num . esc_html__(' Comments','sintec');
            } else {
...
======================================================*/
function sintec_iframe_match(){
    $audio_content = sintec_embedded_media( array('audio', 'iframe') );
...
/*===================================================
	Post embedded media
===================================================*/
function sintec_embedded_media( $type = array() ){
...
/*===================================================
	WP post link pages
====================================================*/
function sintec_link_pages(){
    wp_link_pages( array(
    'before'      => '<div class="page-links"><span class="page-links-title">' . esc_html__( 'Pages:', 'sintec' ) . '</span>',
    'after'       => '</div>',
    'link_before' => '<span>',
    'link_after'  => '</span>',
    'pagelink'    => '<span class="screen-reader-text">' . esc_html__( 'Page', 'sintec' ) . ' </span>%',
...
/*====================================================
	Theme logo
====================================================*/
function sintec_theme_logo( $class = '' ) {
        ...
        <section class="hero-banner hero-banner-sm">
            <div class="container text-center">
				...
				if ( is_category() ) {
					...
					echo '<h2>' . esc_html__( 'Search for: ', 'sintec' ) . get_search_query() . '</h2>';
					...
					echo '<h2>' . esc_html__( 'Blog', 'sintec' ) . '</h2>';

				} elseif ( is_404() ) {
					echo esc_html__( '404 error', 'sintec' );
                ...
                <nav aria-label="breadcrumb" class="banner-breadcrumb">
					<?php
					if ( function_exists( 'sintec_breadcrumbs' ) ) {
						sintec_breadcrumbs();
					}
					?>
                </nav>
...
=================================================*/
function sintec_pull_right( $id = '', $condation ){
...
/*======================================================
	Inline Background
======================================================*/
function sintec_inline_bg_img( $bgUrl ){
    $bg = '';
...
/*======================================================
	Blog Category
======================================================*/
function sintec_featured_post_cat(){
...
========================================================*/
function sintec_sidebar_opt(){

    $sidebarOpt = sintec_opt( 'sintec_blog_layout' );
    $sidebar = '1';
...
============================================================*/
function sintec_contact7_form_content( $template, $prop ) {
...
add_filter( 'wpcf7_default_template', 'sintec_contact7_form_content', 10, 2 );
...
/*============================================================
	Pagination
=============================================================*/
function sintec_blog_pagination(){
	echo '<nav class="blog-pagination justify-content-center d-flex">';
        the_posts_pagination(
            array(
                'mid_size'  => 2,
                'prev_text' => __( '<span class="ti-angle-left"></span>', 'sintec' ),
                'next_text' => __( '<span class="ti-angle-right"></span>', 'sintec' ),
                'screen_reader_text' => ' '
            )
        );
	echo '</nav>';
...
=============================================================*/
if( ! function_exists('sintec_blog_single_post_navigation') ) {
	function sintec_blog_single_post_navigation() {
				...
				<div class="row">
					<div class="col-lg-6 col-md-6 col-12 nav-left flex-row d-flex justify-content-start align-items-center">
						...
						if( get_next_post_link() ){
							...
							if( has_post_thumbnail() ){
								...
								<div class="thumb">
									<a href="<?php the_permalink(  absint( $nextPost->ID )  ) ?>">
										<?php echo get_the_post_thumbnail( absint( $nextPost->ID ), 'sintec_np_thumb', array( 'class' => 'img-fluid' ) ) ?>
									</a>
								</div>
								<?php
							} ?>
							...
							<div class="detials">
								<p><?php echo esc_html__( 'Prev Post', 'sintec' ); ?></p>
								<a href="<?php the_permalink(  absint( $nextPost->ID )  ) ?>">
					...
					<div class="col-lg-6 col-md-6 col-12 nav-right flex-row d-flex justify-content-end align-items-center">
						...
						if( get_previous_post_link() ){
							...
							?>
							<div class="detials">
								<p><?php echo esc_html__( 'Next Post', 'sintec' ); ?></p>
							...
							<div class="thumb">
								<a href="<?php the_permalink(  absint( $prevPost->ID )  ) ?>">
									<?php echo get_the_post_thumbnail( absint( $prevPost->ID ), 'sintec_np_thumb', array( 'class' => 'img-fluid' ) ) ?>
								</a>
							</div>
							<?php
						} ?>
...
/*===================================================
 Sintec Comment Template Callback
 ====================================================*/
function sintec_comment_callback( $comment, $args, $depth ) {
				...
				<div class="desc">
					...
					<div class="d-flex justify-content-between">
						<div class="d-flex align-items-center">
							<h5><?php printf( __( '<span class="comment-author-name">%s</span> ', 'sintec' ), get_comment_author_link() ); ?></h5>
							<p class="date"><?php printf( __('%1$s at %2$s', 'sintec'), get_comment_date(),  get_comment_time() ); ?><?php edit_comment_link( esc_html__( '(Edit)', 'sintec' ), '  ', '' ); ?> </p>
							<?php if ( $comment->comment_approved == '0' ) : ?>
								<em class="comment-awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', 'sintec' ); ?></em>
								<br>
							<?php endif; ?>
						</div>
...
add_filter('comment_reply_link', 'sintec_replace_reply_link_class');
function sintec_replace_reply_link_class( $class ) {
...
===========================================================*/
function sintec_latest_blog( $post_num ){
            ...
            <div class="col-lg-4 col-md-6">
                <div class="single-blog">
                    ...
                    if( has_post_thumbnail() ){
                        ...
                        the_post_thumbnail( 'sintec_latest_blog_360x285' );
                        echo '</div>';
                    }
                    ?>
                    <div class="short_details">
                        <div class="meta-top d-flex">
							<a href="<?php echo esc_url( sintec_blog_date_permalink() ) ?>"><i class="ti-calendar"></i><?php the_time('M j, Y') ?></a>
							<span><i class="ti-folder"></i> <?php echo sintec_featured_post_cat(); ?></span>
                        </div>
...
/*=========================================================
    Share Button Code
===========================================================*/
function sintec_social_sharing_buttons( $ulClass = '', $tagLine = '' ) {
